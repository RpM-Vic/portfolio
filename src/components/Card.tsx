  ...
  return (
    <div className={`flex flex-col carddiv p-4 rounded-2xl ${reverse ? 'md:flex-row-reverse' : 'md:flex-row'} items-center gap-6 mb-10`}>
      <a href={link} target="_blank" rel="noopener noreferrer">
        <img loading='lazy' src={image} alt={title} className="w-48 h-48 md:w-60 md:h-60 object-contain rounded-lg shadow-amber-950" />
      </a>
      <div className="flex-1 p-4
       bg-blue-200 border-2
        hover:bg-gray-100 transition-all
         ...
         ">
        <a href={link} target="_blank" rel="noopener noreferrer">
          <h3 className={`text-2xl font-bold ${link&&"hover:text-blue-500 hover:underline"}`}>{title}</h3>
        </a>
